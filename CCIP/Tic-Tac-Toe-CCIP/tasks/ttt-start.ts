...
task(`ttt-start`, `start a new tic tac toe game`)
  ...
  .setAction(async (taskArguments: TaskArguments) => {
    const { sourceBlockchain, sender, destinationBlockchain, receiver } =
      taskArguments;
    ...
    const tx = await tttDemo.start(destinationChainSelector, receiver);

    await tx.wait();

    spinner.stop();
    console.log(
      `✅ Message sent, game session created! transaction hash: ${tx.hash}`
    );
  });
